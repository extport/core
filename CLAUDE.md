...
This is the **control plane** for the `extport` GitHub org. It manages all mirrored PHP extension repositories, making them installable via [PIE](https://github.com/php/pie) (the PECL replacement).

The org exists because not all PHP extensions have native PIE support yet. We mirror their releases and wrap them in PIE-compatible packaging so users can install them today. This is an independent, unofficial effort — not affiliated with the PHP project or the official PIE team.
...
- It does not build binaries (that's handled by `extport/mirror-action`)
...
```json
{
  "extensions": [
    {
      "name": "redis",
      "mirror-repo": "extport/redis",
      "upstream-repo": "phpredis/phpredis",
      "upstream-type": "github",
      "packagist-name": "extport/redis",
...
5. The mirror repo's workflow calls `extport/mirror-action` to do the actual sync, release, and optional binary build
...
This will:
1. Create `extport/redis` from the `mirror-template` repo
2. Populate `.extport.json` with upstream config
...
2. Enter the mirror repo URL: `https://github.com/extport/<name>`
...
| `EXTPORT_BOT_TOKEN`  | dispatch-syncs, onboarding | needs `workflow` scope on other repos |

The default `GITHUB_TOKEN` cannot trigger workflows in other repos — `EXTPORT_BOT_TOKEN` must be a PAT or GitHub App token with `workflow` scope set as an org secret.

### `EXTPORT_BOT_TOKEN` fine-grained token permissions

The token needs access to **all repositories** in the `extport` org (it creates new repos, pushes to mirrors, and dispatches workflows on any mirror).
...
| **Contents**          | Read & Write | `repos.createOrUpdateFileContents` — write `.extport.json` and `composer.json` in mirror repos |
...
core (this repo)
  ...
  │           └── dispatch-syncs.js
  │           └── workflow_dispatch → extport/redis/sync.yml
  │           └── workflow_dispatch → extport/imagick/sync.yml
  │           └── ...
  │
...
each mirror repo (on dispatch)
  └── sync.yml
        └── extport/mirror-action@v1 (sync)
        └── extport/mirror-action@v1 (release)
        └── extport/mirror-action@v1 (build-binaries → delegates to php/pie-ext-binary-builder)
```

## Build matrix: version-specific PHP constraints

PHP versions for builds are determined exclusively by `php-version-constraints` in `.extport.json`. Each entry maps a semver range of extension versions to the PHP versions to build. First match wins.
...
- [`extport/mirror-action`](https://github.com/extport/mirror-action) — composite action handling sync/release (Node.js) and build (delegates to `php/pie-ext-binary-builder`)
- [`extport/mirror-template`](https://github.com/extport/mirror-template) — template used when creating new mirror repos
